...
cookies, token,loadID = webinfo_salesforce.get_login_parameters()

queueID = webinfo_salesforce.get_queueID()
userID = webinfo_salesforce.get_userID()

take_list = webinfo_salesforce.get_take_list()
        ...
        try:
            ...
            if length==7:  #len(text2["context"]["globalValueProviders"]) == 6 代表有case，等于5代表没有case
                ...
                for key in text2["context"]["globalValueProviders"][6]["values"]["records"].keys():
                    ...
                    if (case_number[len(case_number)-1] in take_list) and (case_district == 'APAC'):
